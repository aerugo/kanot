<script>
  ...
  import { codeTypes, codes } from "../stores/codeStore.js";
  import { addCode } from "../utils/api.js";
  ...
  async function handleSubmit() {
    try {
      const response = await addCode(newCode);
      if (response && response.code_id) {
        // Ensure the response contains the new code
        codes.add(response); // Add the new code to the store
        resetForm();
        isFormVisible = false;
  ...
  <section class="add-code" transition:slide={{ duration: 300 }}>
    ...
    <form on:submit|preventDefault={handleSubmit}>
      ...
      <div class="form-buttons">
        ...
        <button type="button" class="cancel-btn" on:click={toggleForm}
          >Cancel</button
        >
      </div>
    </form>
