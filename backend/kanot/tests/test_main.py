...
def test_create_project(test_client, test_db) -> None:
    response = test_client.post(
        "/projects/",
        json={"project_title": "API Test Project", "project_description": "API Test Description"}
    )
    assert response.status_code == 200
    data = ProjectResponse.model_validate(response.json())
    assert data.project_title == "API Test Project"
    assert data.project_description == "API Test Description"
...
def test_read_project(test_client, test_db) -> None:
    ...
    create_response = test_client.post(
        "/projects/",
        json={"project_title": "API Test Project 2", "project_description": "API Test Description 2"}
    )
    ...
    assert response.status_code == 200
    data = ProjectResponse.model_validate(response.json())
    assert data.project_title == "API Test Project 2"
    assert data.project_description == "API Test Description 2"

def test_update_project(test_client, test_db) -> None:
    # Create a project
    create_response = test_client.post(
        "/projects/",
        json={"project_title": "API Test Project 3", "project_description": "API Test Description 3"}
    )
    project_data = ProjectResponse.model_validate(create_response.json())
...
def test_delete_project(test_client, test_db) -> None:
    ...
    create_response = test_client.post(
        "/projects/",
        json={"project_title": "API Test Project 4", "project_description": "API Test Description 4"}
    )
    project_data = ProjectResponse.model_validate(create_response.json())
...
def test_create_code_type(test_client, test_db) -> None:
    ...
    response = test_client.post(
        "/code_types/",
        json={"type_name": "API Test CodeType", "project_id": project_data.project_id}
    )
    assert response.status_code == 200
    data = CodeTypeResponse.model_validate(response.json())
    assert data.type_name == "API Test CodeType"
...
def test_create_code(test_client, test_db) -> None:
    ...
    response = test_client.post(
        ...
        json={
            ...
            "reference": "API Test Reference",
            "coordinates": "API Test Coordinates",
            "project_id": project_data.project_id
        }
    )
    assert response.status_code == 200
    data = CodeResponse.model_validate(response.json())
    assert data.term == "API Test Code"
...
def test_create_series(test_client, test_db) -> None:
    ...
    response = test_client.post(
        "/series/",
        json={"series_title": "API Test Series", "project_id": project_data.project_id}
    )
    assert response.status_code == 200
    data = SeriesResponse.model_validate(response.json())
    assert data.series_title == "API Test Series"
...
def test_create_segment(test_client, test_db) -> None:
    ...
    # Create a segment
    unique_segment_title = f"API Test Segment {uuid.uuid4()}"
    response = test_client.post(
        "/segments/",
...
def test_create_element(test_client, test_db) -> None:
    ...
    response = test_client.post(
        "/elements/",
        json={"element_text": "API Test Element", "segment_id": segment_data.segment_id, "project_id": project_data.project_id}
    )
    assert response.status_code == 200
    data = ElementResponse.model_validate(response.json())
    assert data.element_text == "API Test Element"
